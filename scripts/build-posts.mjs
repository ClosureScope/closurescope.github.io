...
export function buildPosts() {
  ...
  const posts = readdirSync(join(root, 'posts')).filter(name => name.endsWith('.typ')).sort().map(name => {
    ...
    const { title, date, description } = metadata[0];
    if (![title, date, description].every(value => typeof value === 'string' && value.trim())) {
      throw new Error(`${name} 的 title、date、description 必须是非空字符串。`);
    }
    ...
    return { slug, title, date, description, body, styles };
  });
